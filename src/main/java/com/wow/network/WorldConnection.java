...
import com.wow.entities.packet.IPacketReadable;
import com.wow.entities.packet.IPacketWritable;
import com.wow.entities.packet.Packet;
import com.wow.entities.packet.ServerPacket;
import com.wow.enums.ClientVersion;
import com.wow.logon.RealmAuth;
...
/**
 * 
 * A connection between the gameserverpackets and the world, handles all incoming packets for all WoW versions (<= MoP)
 *
 * @author Marijn
 * 
 * 
...
public class WorldConnection extends Connection{
        ...
        try{
            ...
            while((readByte = (byte) in.read()) != -1){
                ...
                try {
                	...
                	ClientPacket pTemp = (ClientPacket) Class.forName("com.wow.network.gameserverpackets." + p.sOpcode).newInstance();
                	pTemp.put(p);
                	p = pTemp;
					try {
        	...
        	// The gameserverpackets parent might be null if the realm authentication hasn't been completed yet
        	if(client != null)
    ...
    private ClientPacket readPacket(byte firstByte){
        ...
        try {
            ...
            Logger.writeLog("Couldn't read gameserverpackets packet", Logger.LOG_TYPE_WARNING);
        }
        return p;
    }
    ...
    private void decodeHeader(Packet p){
    	// Client parent isn't null so gameserverpackets is authenticated
    	if (client != null)
